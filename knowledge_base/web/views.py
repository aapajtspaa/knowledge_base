...
from .models import Zhibingjili, Bingjixibie, Bingzheng, Lifang
import time
...
def bingji(request):
    return render(request, 'bingji.html')


def zhifa(request):
    return render(request, 'zhifa.html')


def fangji(request):
    return render(request, 'fangji.html')


def jiegou(request):
    return render(request, 'jiegou.html')


def shiyong(request):
    return render(request, 'shiyong.html')
...
def piwei(request):
    return render(request, 'piwei.html')

def liangzang(request):
    return render(request, 'liangzang.html')

def shenxi(request):
    return render(request, 'shenxi.html')

def gandan(request):
    return render(request, 'gandan.html')

def xinxi(request):
    return render(request, 'xinxi.html')

def feixi(request):
    return render(request, 'feixi.html')

def zhishiku(request):
    return render(request, 'zhishiku.html')
...
def bingjixibie(request):
    names = Bingjixibie.objects.filter()
    return render(request, 'bingjixibie.html', {'names': names})


def zhibingjili(request):
    names = Zhibingjili.objects.filter()
    return render(request, 'zhibingjili.html', {'names': names})


def bingzheng(request):
    names = Bingzheng.objects.filter()
    return render(request, 'bingzheng.html', {'names': names})


def lifang(request):
    names = Lifang.objects.filter()
    return render(request, 'lifang.html', {'names': names})


def search_form(request):
    # names = Zhibingjili.objects.filter()
    # return render(request, 'search_form.html', {'names': names})
...
def search1(request):
    ...
    if 'q' in request.GET and request.GET['q']:
        file = open('./static/test1/待分类文本/1.txt', 'w+', encoding='UTF-8')
        a = request.GET['q']
        print(a, file=file)
        file.close()

        file = open('./static/test1/待分类文本/1.txt', 'r', encoding='UTF-8')
        sent = file.read()
        sent_list = jieba.cut(sent)
        #
        file0 = open('./static/split/test_split/待分类文本/1.txt', 'w+', encoding='UTF-8')
        ...
        print(end - start)
        content = {"病状类别": resultReturn, "总计用时": str(end - start) + 's', 'q': request.GET['q']}
    else:
        content = {"病状类别": '在搜索框输入病状，并单击搜索'}
    return render(request, 'jieguo.html', content)
    # return HttpResponse(message)


datapath = "./static/data/"  # 原始数据路径
stopWord_path = "./static/stop/stopword.txt"  # 停用词路径
test_path = "./static/test1/"  # 测试集路径
test_split_dat_path = "./static/test_set.dat"  # 测试集分词bat文件路径
testspace_dat_path = "./static/testspace.dat"  # 测试集输出空间矩阵dat文件
train_dat_path = "./static/train_set.dat"  # 读取分词数据之后的词向量并保存为二进制文件
tfidfspace_dat_path = "./static/tfidfspace.dat"  # tf-idf词频空间向量的dat文件
test_split_path = './static/split/test_split/'  # 测试集分词路径
split_datapath = "./static/split/split_data/"  # 对原始数据分词之后的数据路径
testSpace_path = "./static/testSpace.txt"  # 测试集分词信息
testSpace_arr_path = "./static/testSpace_arr.txt"  # 测试集词频矩阵信息
trainbunch_vocabulary_path = "./static/trainbunch_vocabulary.txt"  # 所有分词词频信息
tfidfspace_out_arr_path = "./static/tfidfspace_out_arr.txt"  # tfidf输出矩阵信息
tfidfspace_out_word_path = "./static/tfidfspace_out_word.txt"  # 单词形式的txt
testspace_out_arr_path = "./static/testspace_out_arr.txt"  # 测试集输出矩阵信息
testspace_out_word_apth = "./static/testspace_out_word.txt"  # 测试界单词信息
